...
import { ICheckValidUsername as DataForServer } from 'src/api/endpoints/auth';

export const useValidateUsernameForm = <FormInputs>() => {
  ...
  const sendRequest = useCallback(async (dataForServer: DataForServer) => {
    setIsLoading(true);
    try {
      const response: AxiosResponse<any> = await API.endpoints.auth.checkValidUsername(
        dataForServer
      );
