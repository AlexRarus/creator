...
import { ISetPassword as DataForServer } from 'src/api/endpoints/auth';

export const useSubmitSetPasswordForm = <FormInputs>() => {
  ...
  const sendRequest = useCallback(async (dataForServer: DataForServer) => {
    setIsLoading(true);
    try {
      const response: AxiosResponse<any> = await API.endpoints.auth.setPassword(dataForServer);
