...
import { IUpdateUserIndexPage as DataForServer } from 'src/api/endpoints/auth';

export const useSubmitUserIndexPageForm = <FormInputs>() => {
  ...
  const sendRequest = useCallback(async (dataForServer: DataForServer) => {
    setIsLoading(true);
    try {
      const response: AxiosResponse<any> = await API.endpoints.auth.updateUserIndexPage(
        dataForServer
      );
